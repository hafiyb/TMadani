export const malayLang = {
  home: {
    text1:
      'Tugas Kerajaan adalah untuk memastikan tiada rakyat yang tertinggal atau diabaikan.',
    section1_text1:
      'Parti-parti Kerajaan Perpaduan percaya kepada layanan adil dan saksama untuk Kedah, Kelantan, Negeri Sembilan, Pulau Pinang, Selangor dan Terengganu.',
    section1_text2:
      'Ini bermaksud memudahcara pembangunan potensi setiap negeri dengan mencipta peluang ekonomi setempat yang relevan.',
    section1_text3:
      'Tiada anak negeri yang sepatutnya meninggalkan kampung halaman hanya untuk mencapai keselesaan dan kebahagiaan.',
    section1_text4:
      'Menerusi teknologi dan penghakupayaan keusahawanan individu, kami percaya kami mampu menyemai benih kemakmuran merentasi enam negeri ini.',
    section1_text5: 'Ketahui caranya',
    section2_text1:
      'Walaupun sebagai kerajaan yang memerintah, kami percaya bahawa adalah menjadi tanggungjawab kami untuk menjalankan kempen yang berprinsip.',
    section2_text2:
      'Maka, kempen kami tidak akan menggunakan wang pembayar cukai atau sumber Kerajaan Persekutuan.',
    section2_text3:
      'Pada tahun 2022, CISTA menggerakkan 40,000 orang sukarelawan yang membantu dalam kempen, pengumpulan dana, dan latihan untuk menjadi PACA.',
    section2_text4:
      'Kami memerlukan sokongan anda sekali lagi - muat turun Aplikasi CISTA dan sertai aktiviti yang berdekatan dengan anda.',
  },
  volunteer: {
    section1_text1: 'Tawaran Madani adalah digerakkan oleh rakyat seperti anda',
    section1_text2:
      'If you would like to join our field teams — calling fence-sitters & canvassing, training to become PACAs — download the CISTA App and register today. Perpaduan is driven by people like you.',
  },
  pencapaian: {
    section1_text1:
      'Kerajaan Madani telah memimpin Malaysia selama dua suku tahun.',
    section1_text2:
      'Dalam tempoh itu, beberapa inisiatif telah dilaksanakan untuk menangani tekanan ekonomi yang kini membeban rakyat Malaysia.',
    section1_text3:
      'Senarai berikut menyatakan 40 inisiatif yang telah dilaksanakan oleh Kerajaan Madani demi mencapai peningkatan pekerjaan bermaruah, gaji bermaruah, dan kehidupan bermaruah di negara kita.',
    section2_text1:
      'Jika objektif-objektif dasar ini dirasai dan bersesuaian dengan anda, undilah perpaduan.',
    s1: 'Pekerjaan',
    s2: 'Kos Sara Hidup',
    s3: 'Infrastruktur',
    s4: 'Kebajikan',
    s1q1: 'Inisiatif Pendapatan Rakyat (IPR)',
    s1q2: 'Inisiatif Kemudahan Rakyat (IKR)',
    s1q3: 'Penyerapan 4,914 Doktor Kontrak ke Jawatan Tetap',
    s1q4: 'Inisiatif ‘Academy in Factory’',
    s1q5: 'Perkongsian Keuntungan BERNAS dengan Petani Padi',
    s1q6: 'Pelan Pembangunan Semula Lapangan Terbang Subang',
    s1q7: 'Pelaburan RM25.5 bilion dari AWS dan Tesla',
    s1q8: 'Pelaburan RM30 bilion dari Geely',
    s1q9: 'Penamatan Monopoli PUSPAKOM dan MyEG',
    s1q10: 'Geran Pendigitalan PKS',
    s2q1: 'Pembekuan Tarif Elektrik untuk Rumah dan Perniagaan Kecil',
    s2q2: 'Inisiatif Payung Rahmah',
    s2q3: 'Penjaminan Bekalan Telur',
    s2q4: 'Pengurangan Cukai Pendapatan M40 sebanyak 2%',
    s2q5: 'Insentif Sumbangan Tunai Rahmah (STR)',
    s2q6: 'Skim Perubatan Madani',
    s2q7: 'Pengurangan Tol BESRAYA dan LEKAS sebanyak 8%',
    s2q8: 'Perjalanan KTM Percuma untuk Pelajar Sekolah Rendah dan OKU',
    s2q9: 'Penurunan Tarif Gas 2023-2025',
    s2q10: 'Kawalan Harga dan Kapasiti pada Musim Perayaan',
    s3q1: '40 Projek Mitigasi Banjir di Kelantan & Terengganu',
    s3q2: '47% dari Keseluruhan Projek PR1MA ‘Sakit’ Disiapkan dalam 5 Bulan',
    s3q3: '7,000 Unit Rumah Mampu Milik di bawah SASaR',
    s3q4: 'Peruntukan Kecemasan untuk PPR ‘Sakit’',
    s3q5: 'Perlihan Tenaga Solar untuk Bangunan Kerajaan',
    s3q6: 'Pam Bergerak untuk Mengatasi Banjir Termenung',
    s3q7: 'RM1.5 bilion untuk Infrastruktur Air di Kelantan',
    s3q8: 'Penjimatan RM11 bilion untuk Projek ECRL',
    s3q9: 'RM2.8 bilion untuk Penggantian Bas/Tren Prasarana',
    s3q10: '7 Stesen Pengecasan Baru di Lebuhraya PLUS dan LPT',
    s4q1: 'Bekalan Makanan 21,000kg untuk Orang Asli',
    s4q2: 'Kewarganegaraan Melalui Ibu Malaysia',
    s4q3: 'Dasar ‘Kill Switch’ untuk Bank',
    s4q4: 'Dasar ‘Peluang Kedua’ untuk Individu Muflis',
    s4q5: 'Tambahan 289 Sekolah yang Menawarkan Program Pendidikan Khas Integrasi (PPKI)',
    s4q6: 'Pengujian Keselesaan untuk Rumah Mampu Milik',
    s4q7: 'Pengawalan Vape',
    s4q8: 'Pengaktifan Semula Majlis Pembangunan Bumiputera Pulau Pinang (PBDC)',
    s4q9: 'Peruntukan RM100 juta untuk Tindak Balas Banjir',
    s4q10: 'Vaksin Pneumokokal Percuma',
    s1q1a1:
      'Kerajaan Madani telah memperkenalkan Inisiatif Pendapatan Rakyat (IPR) untuk melahirkan peluang pekerjaan yang menjana sekurang-kurangnya RM2,000 sebulan untuk miskin tegar',
    s1q1a2:
      'IPR dibahagi kepada 3 komponen. ‘INTAN’ meliputi geran tanah, peralatan, dan pelatihan kepada pekebun kecil, dan elaun tambahan agar mereka mencapai pendapatan RM2,000 sebulan sehingga musim penuaian.  ',
    s1q1a3:
      '‘INSAN’ meliputi geran mesin menjual (vending machine) dan pengecualian sewa di kawasan sibuk seperti hab pengangkutan, universiti, atau hospital. Vendor makanan dapat menggunakan mesin tersebut untuk jual produk masing-masing tanpa kos.',
    s1q1a4:
      '‘IKHSAN’ meliputi penyumberan luar (outsourcing) kerja ‘3D’ (dirty, dangerous, difficult) kepada warganegara yang berkemampuan. Ini dilaksanakan dengan menawarkan kerja ‘3D’ secara terus kepada pemohon, dan bukan melalui agensi pekerjaan yang beruntung dengan menggunakan tenaga kerja warga asing. Pekerja IKHSAN juga akan dibenar untuk simpan kutipan fi dari penggunaan fasiliti awam (cth. tandas awam).',
    s1q1a5: 'Pemohonan IPR boleh dibuat di https://ipr.epu.gov.my/sign_up.',
    s1q2a1:
      'Kerajaan Madani telah membuka peluang pekerjaan kepada kontraktor kecil melalui Inisiatif Kemudahan Rakyat (IKR).',
    s1q2a2:
      'IKR memberi peluang kepada kontraktor kecil untuk melapor keperluan penyelenggaraan, penambahbaikan, atau pembinaan di kawasan tempatan mereka. Setelah mendapat kelulusan, kontraktor tersebut bakal mendapat peruntukan untuk melaksanakan projek infrastruktur tersebut.',
    s1q2a3:
      'Pemohonan IKR boleh dibuat di https://ikr.ekonomi.gov.my/register.',
    s1q3a1:
      'Seramai 4,914 doktor kontrak telah diserap ke jawatan tetap untuk tahun 2023 - termasuk 4,263 pegawai perubatan, 335 pegawai dental, dan 316 pegawai farmasi.',
    s1q3a2:
      'Demi membiayai penyerapan tersebut, Bajet 2023 telah memberi tambahan emolumen sebanyak RM3 bilion kepada Kementerian Kesihatan Malaysia (KKM).',
    s1q3a3:
      'Kerajaan Madani akan membuka 12,800 jawatan tetap tambahan sebelum akhir tahun 2026.',
    s1q4a1:
      'Kerajaan Madani telah melancarkan program pelatihan Academy in Factory (AIF), iaitu inisiatif yang bertuju untuk memberi pengalaman dan pekerjaan kepada 100,000 pemuda lulusan TVET.',
    s1q4a2:
      'Melalui kerjasama dengan pemain industri, AIF memberi lulusan TVET pelatihan yang relevan dalam sektor pembuatan, sebelum diserap oleh majikan.',
    s1q5a1:
      'Pada Disember 2022, Kerajaan Madani telah mengarahkan BERNAS untuk memyumbang keuntungan sebanyak RM10 juta kepada petani padi, bermula dengan petani di Kelantan, Terengganu, dan Pahang.',
    s1q5a2:
      'Kerajaan Madani juga telah mengarahkan BERNAS untuk meningkatkan sumbangan ini kepada RM50 juta mulai dari tahun 2023. BERNAS juga akan mula menyumbang 30% dari untung bersih kepada petani padi.',
    s1q6a1:
      'Kerajaan Madani telah meluluskan penyambungan semula servis jet komersial dari Lapangan Terbang Subang (untuk kali pertama sejak tahun 2002).',
    s1q6a2:
      'Inisiatif ini akan menjana jejak kaki (footfall) yang lebih kerap di kawasan sekeliling, dan juga akan menjana peluang pelaburan dan pekerjaan dalam sektor penyelenggaraan, pembaikan dan baik pulih (MRO), penyelidikan penerbangan, dan operasi penerbangan serantau.',
    s1q7a1:
      'Kerajaan Madani berjaya menarik pelaburan sebanyak RM25.5 bilion dari Amazon Web Services (AWS) dan Tesla, yang akan menyumbang kepada penjanaan beribu-ribu kerja dalam sektor IT, teknologi, dan kerja kolar putih lain yang relevan.',
    s1q7a2:
      'Pelaburan tersebut akan menjadi peneraju untuk pembangunan Malaysia sebagai hab teknologi serantau, lalu memanfaatkan kedudukan strategik dan dasar berkecuali negara.',
    s1q8a1:
      'Kerajaan Madani berjaya menarik pelaburan sebanyak RM30 bilion dari Geely yang berkenaan dengan pembangunan Lembah Teknologi Tinggi Automotif (AHTV) di Tanjong Malim.',
    s1q8a2:
      'Fasiliti AHTV akan menyumbang kepada penubuhan Malaysia sebagai hab serantau penyelidikan dan servis kereta elektrik (EV).',
    s1q9a1:
      'Kerajaan Madani telah menamatkan hak eksklusif PUSPAKOM untuk menjalankan pemeriksaan kenderaan dan hak eksklusif MyEG untuk menyediakan servis e-kerajaan.',
    s1q9a2:
      'Persaingan yang bakal wujud akan bantu mengurang tempoh masa tunggu untuk pengguna dan memberi peluang kepada bisnes kecil yang mampu menawarkan servis yang relevan.',
    s1q10a1:
      'Kerajaan Madani telah meluluskan RM100 juta di bawah Geran Pendigitalan PKS untuk mendorong automasi dan pendigitalan aktiviti-aktiviti perniagaan.',
    s1q10a2:
      'Geran sehingga RM5,000 akan diberi kepada PKS yang menggunakan produk atau servis digital, lalu menyumbang kepada peningkatan daya saing perniagaan masing-masing. Tambahan pula, RM1 bilion telah diagih melalui Bank Negara Malaysia untuk membantu mikro-PKS.',
    s2q1a1:
      'Kerajaan Madani telah membelanjakan RM10.7 bilion dari Januari hingga Jun demi subsidi elektrik mikro-PKS, restoran, peruncit, bengkel, dan pekebun kecil.',
    s2q1a2:
      'Inisiatif tersebut berjaya menjamin 9.5 juta pengguna elektrik daripada peningkatan bil elektrik.',
    s2q2a1:
      'Kerajaan Madani telah memperkenalkan inisiatif Payung Rahmah pada bulan Januari. Melalui kerjasama dengan pemain industri, Kerajaan telah berusaha untuk meluaskan bekalan produk dan servis berkemampuan kepada rakyat miskin tegar.',
    s2q2a2:
      'Antara inisiatif di bawah Payung Ramah termasuk set makanan (Menu Rahmah), bakul makanan (Bakul Rahmah), rawatan medikal (Klinik Rahmah), dan produk optik (Pakej Rahmah).',
    s2q3a1:
      'Kerajaan Madani telah melonggarkan larangan terhadap import telur asing pada bulan Disember agar menangani kekurangan bekalan daripada pembekal tempatan. Langkah ini juga digabungkan dengan kawalan harga untuk kekalkan harga makanan. Langkah tersebut dikuatkuasakan sehingga Januari, sebelum diperkenalkan semula pada bulan Julai untuk menangani kekurangan terkini.',
    s2q3a2:
      'Kerajaan Madani tidak pernah memberi subsidi untuk import telur asing, dan masih menyokong pembekal tempatan dengan subsidi eksklusif. Kerajaan sedang terus mengkaji usaha berpatutan untuk menangani masalah keterjaminan makanan secara long-term.',
    s2q4a1:
      'Kerajaan Madani telah meluluskan pengurangkan cukai pendapatan sebanyak 2% untuk mereka yang mempunyai gaji RM35,000 hingga RM100,000 setahun.',
    s2q4a2:
      'Ini membolehkan individu tertentu untuk jimat sebanyak RM2,300 dalam taksiran cukai.',
    s2q5a1:
      'Kerajaan Madani telah mengenalkan bantuan kewangan di bawah inisiatif STR.',
    s2q5a2:
      'Isi rumah yang mempunyai pendapatan kurang dari RM2,500 sebulan akan terima sehingga RM2,500 bantuan tunai (berdasarkan jumlah anak). Isi rumah yang mempunyai pendapatan antara RM2,501 hingga RM5,000 sebulan akan terima sehingga RM1,250 bantuan tunai (berdasarkan jumlah anak).',
    s2q5a3:
      'Bujang juga akan terima antara RM350 hingga RM600 berdasarkan umur.',
    s2q6a1:
      'Kerajaan Madani telah memperkenalkan Skim Perubatan Madani untuk penerima STR.  Melalui Skim Perubatan Madani, penerima STR layak mendapat perundingan, pemeriksaan, perubatan, dan prosedur secara percuma.  ',
    s2q6a2:
      'Skim Perubatan Madani sedang melalui fasa perintis dengan peruntukan RM100 juta.',
    s2q7a1:
      'Kerajaan Madani telah menurunkan kadar tol di lebuhraya BESRAYA dan LEKAS sebanyak 8%. Langkah ini bertuju untuk menurunkan pembelanjaan pengangkutan untuk komuter yang lalu antara Selangor dan Negeri Sembilan.',
    s2q8a1:
      'Kerajaan Madani telah memperkenalkan perjalanan percuma di servis KTM Komuter dan Antarabandar untuk pelajar yang berumur 12 ke bawah, dan juga orang kelainan upaya.',
    s2q8a2:
      'Kos untuk skim tersebut ditanggung oleh penjimatan RM15 juta yang diperolehi dengan pembatalan servis KTM Subang Skypark.',
    s2q9a1:
      'Kerajaan Madani telah mengurangkan tarif purata asas penggunaan kemudahan gas yang perlu dibayar oleh pengguna antara tahun 2023 hingga 2025.',
    s2q9a2:
      'Terpulang kepada jumlah penggunaan, penurunan tarif tersebut bakal menjana penjimatan sebanyak RM4,000 hingga RM55.9 juta untuk tahun 2023.',
    s2q10a1:
      'Melalui kerjasama dengan pemain industri, Kerajaan Madani telah berjaya menawarkan sebanyak 25,020 tiket penerbangan tambahan pada musim perayaan Januari, dengan harga dari RM199 sahaja.',
    s2q10a2:
      'Semasa musim Aidilfitri, Kerajaan Madani juga telah menawar 11,400 tiket penerbangan tambahan dari RM199, dan 13,860 tiket ETS tambahan.',
    s2q10a3:
      'Semasa musim Tahun Baru Cina, Kerajaan Madani juga telah melulus 291 penerbangan tambahan demi meningkatkan kapasiti dan mengurangkan tambang.',
    s3q1a1:
      'Pada bulan Disember, Kerajaan Madani telah meluluskan 40 projek mitigasi banjir dan saliran air di Kelantan dan Terengganu.',
    s3q1a2:
      '2 projek utama - iaitu Projek Pembangunan Lembangan Sungai Bersepadu (PLSB) di Sungai Kelantan dan PLSB di Sungai Golok - dijangka siap sebelum Oktober 2023 dan Ogos 2024.',
    s3q1a3:
      'Pelaksanaan projek tersebut bertujuan untuk menjamin lebih ramai penduduk tempatan daripada kesan banjir-banjir yang telah berlaku.',
    s3q2a1:
      'Kerajaan Madani telah campur tangan dengan pihak berkenaan untuk mempercepatkan penyiapan projek rumah mampu milik PR1MA yang ‘sakit’.',
    s3q2a2:
      'Antara Januari hingga Mei 2023, Kerajaan Madani telah berjaya menyiapkan 8 dari 17 projek PR1MA yang berstatus ‘sakit’. Baki projek ‘sakit’ dijangka siap pada tahun 2024.',
    s3q3a1:
      'Kerajaan Madani telah menyumbang RM500 juta untuk menyelenggara dan membina rumah mampu milik untuk Angkatan Tentera Malaysia, termasuk 7,000 unit rumah mampu milik baru.',
    s3q4a1:
      'Kerajaan Madani telah menyumbang RM35 juta dan menggunakan kaedah tender terhad untuk inisiatif PPR ‘Kita Untuk Kita’.',
    s3q4a2:
      'Inisiatif terhadap bertujuan untuk mempercepatkan pembaikan infrastruktur PPR yang makin lemah di negeri Selangor dan Kuala Lumpur.',
    s3q5a1:
      'Kerajaan Madani sedang menaik tarafkan dan mengubahsuai bangunan kerajaan dengan sistem solar.',
    s3q5a2:
      'Langkah ini akan memastikan majoriti penggunaan elektrik kerajaan dijana secara sendiri dan secara hijau sebelum hujung 2023, lalu mengurangkan beban yang ditanggung oleh pembayar cukai.',
    s3q6a1:
      'Pada Disember, Kerajana Madani telah berbelanja RM12 juta untuk belian enam unit pam bergerak demi Kerajaan Negeri Kelantan.  Langkah ini berjaya mengatasi masalah air banjir bertakung di Pasir Mas selepas banjir hujung tahun 2022.',
    s3q7a1:
      'Kerajaan Madani telah meluluskan peruntukan RM1.5 bilion untuk menyelesaikan masalah bekalan air di negeri Kelantan.',
    s3q8a1:
      'Kerajaan Madani mengumumkan pengurangkan kos pembangunan ECRL dari RM85.97 bilion ke RM74.96 bilion, setelah menggunakan kaedah total value engineering (VM).',
    s3q8a2:
      'Projek ECRL mampu menambah rangkaian pengangkutan di kawasan 665km antara Kota Bharu dan Pelabuhan Klang.',
    s3q9a1:
      'Kerajaan Madani telah memberi peruntukkan tambahan sebanyak RM2.8 bilion kepada Prasarana Malaysia, agar menggantikan peralatan tren dan bas serta meningkatkan stok alat ganti.',
    s3q10a1:
      'Di bawah Kerajaan Madani, RM90 juta telah diperuntukkan untuk membina 7 stesen pengecasan baru untuk kereta elektrik (EV) di Lebuhraya Utara-Selatan.',
    s3q10a2:
      'Ini merupakan tambahan kepada 3 stesen pengecasan EV yang sedia ada di Ayer Keroh, Tapah, dan Paka.',
    s4q1a1:
      'Kerajaan Madani telah meluluskan bekalan makanan sebanyak 21,000kg untuk diagih kepada 1,400 keluarga Orang Asli di Kelantan.',
    s4q1a2:
      'Pengedaran bekalan makanan tersebut telah dilaksanakan oleh Jabatan Bomba dan Penyelamat Malaysia.',
    s4q2a1:
      'Kerajaan Madani telah mula proses penggubalan Bahagian III dalam Perlembagaan Persekutuan, agar anak-anak yang dilahir di luar negara kepada ibu warganegara Malaysia bakal mendapat kewarganegaraan.',
    s4q2a2:
      'Selain itu, Kerajaan Madani juga telah menyelesaikan 8,288 dari 10,000 kes rayuan kewarganegaraan.',
    s4q3a1:
      'Kerajaan Madani telah mengarahkan Bank Negara Malaysia untuk menguatkuasakan dasar ‘kill switch’ untuk semua institusi bank. ‘Kill switch’ tersebut memberi peluang kepada pelanggan untuk membekukan akaun dan kad mereka sekiranya ada aktiviti mencurigakan.',
    s4q3a2:
      'Langkah tersebut diambil sebagai tindakan untuk lawan scam dan penipuan online.',
    s4q4a1:
      'Kerajaan Madani telah melaksanakan dasar ‘Peluang Kedua’ untuk individu muflis di Malaysia.',
    s4q4a2:
      'Kes kebankrapan di bawah jumlah RM50,000 telah dilepas secara automatik, dan pindaan kepada Akta Insolvensi 1967 akan dilaksanakan agar mempercepatkan pelepasan kes kebankrapan pada masa hadapan.',
    s4q5a1:
      'Kerajaan Madani telah meningkatkan jumlah sekolah rendah dan sekolah menengah yang menawarkan Program Pendidikan Khas Integrasi (PPKI) dari 2,343 ke 2,632.',
    s4q5a2:
      'Langkah ini akan memastikan tiada kanak-kanak kurang upaya tercicir mendapat pendidikan sewajarnya.',
    s4q6a1:
      'Kerajaan Madani telah memutus bahawa 220,000 rumah mampu milik yang baru atau sedang dibina perlu lulus beberapa pengujian saintifik yang berkaitan dengan keselesaan.',
    s4q6a2:
      'Antara penilaian tersebut termasuk pengujian terowongan angin (windtunnel), analisis sun-path, dan keselesaan terma.',
    s4q7a1:
      'Kerajaan Madani telah menghalalkan vape, lalu menyediakan rangka kerja untuk kawalan lebih mendalam mengenai pengedaran peranti dan produk berkaitan.',
    s4q7a2:
      'Langkah ini akan memberi peluang kepada Kerajaan untuk menguatkuasakan taraf kesihatan dan keselamatan minima terhadap produk tersebut, lalu membenarkan pemantauan lebih mendalam.',
    s4q8a1:
      'Kerajaan Madani telah aktifkan semula Majlis Pembangunan Bumiputera Pulau Pinang (PBDC) di bawah Jabatan Perdana Menteri.',
    s4q8a2:
      'PBDC akan bekerjasama dengan Kerajaan Negeri Pulau Pinang untuk mempercepatkan pembangunan komuniti Melayu di Pulau Pinang, dengan fokus kepada pembangunan tanah wakaf dan pemilikan harta.',
    s4q9a1:
      'Kerajaan Madani telah memberi peruntukan RM100 juta kepada Kelantan dan RM100 juta kepada Terengganu untuk membiayai bantuan kepada mangsa-mangsa banjir bulan Disember.',
    s4q10a1:
      'Kerajaan Madani telah memperkenalkan vaksin pneunookal percuma untuk kanak-kanak yang lahir antara tahun 2018 dan 2019.',
    s4q10a2:
      'Inisiatif ini bertujuan untuk membantu kanak-kanak yang tidak terjamin di bawah Program Imunisasi Kebangsaan Malaysia sebelum ini.',
  },
  volunteer: {
    section1_text1: 'Tawaran Madani adalah digerakkan oleh rakyat seperti anda.',
    section1_text2:
      'Sekiranya anda ingin menyertai pasukan lapangan kami — menghubungi pengundi atas pagar & berjumpa mereka, atau dilatih untuk menjadi PACA — muat turun aplikasi CISTA dan daftar hari ini.',
  },
  faq: {
    s1q1: 'How can I register as a volunteer for the Undi Perpaduan campaign?',
    s1q2: 'I have registered as a volunteer. What activities can I do after this?',
    s2q1: 'Bagaimanakah saya boleh memuat turun CISTA?',
    s2q2: 'Saya mengalami masalah untuk mendaftar aplikasi CISTA.',
    s2q3: 'Saya tidak boleh log masuk ke dalam aplikasi CISTA.',
    s2q4: 'Pendaftaran aplikasi CISTA mengambil masa yang lama.',
    s2q5: 'Kenapa pendaftaran CISTA memerlukan kad pengenalan? ',
    s2q6: 'Saya menghadapi kesukaran untuk memuat turun aplikasi CISTA luar daripada Malaysia.',
    s1a1: 'To become a volunteer, you are required to register as a volunteer via the CISTA app.',
    s1a2: 'Any latest activities regarding Undi Perpaduan will be updated on the CISTA app.',
    s2a1: 'CISTA boleh dimuat turun melalui App Store, Play Store & Huawei App Gallery.',
    s2a2: "Pastikan anda memasukkan nombor telefon & nombor kad pengenalan yang sah sebelum menekan butang 'Daftar Akaun'.",
    s2a3: "Pastikan nombor telefon dan kata laluan yang dimasukkan adalah tepat sebelum anda menekan butang 'Log Masuk'.",
    s2a4: 'Pastikan anda mempunyai rangkaian kelajuan internet yang stabil.',
    s2a5: 'Kenapa pendaftaran CISTA memerlukan kad pengenalan? ',
    s2a6: 'Aplikasi CISTA hanya didapati dan boleh dimuat turun di Malaysia sahaja buat masa kini.',
  },
};

export const engLang = {
  home: {
    text1: 'The job of government is to ensure that no one is left behind.',
    section1_text1:
      'The parties of the Unity Government believe in fair and equitable treatment of Kedah, Kelantan, Negeri Sembilan, Pulau Pinang, Selangor, and Terengganu.',
    section1_text2:
      'This means facilitating each state’s unique potential, by creating economic opportunities that are locally relevant.',
    section1_text3:
      'No anak negeri should have to leave home in order to achieve prosperity and happiness.',
    section1_text4:
      'By using technology and empowering enterprising individuals, we believe we can lay these seeds of prosperity across the 6 states.',
    section1_text5: 'Find out how',
    section2_text1:
      'Even as the ruling government, we believe it is our responsibility to run a principled campaign.',
    section2_text2:
      'So our campaign doesn’t use taxpayer money or federal resources.',
    section2_text3:
      'In 2022, CISTA mobilised 40,000 volunteers who helped campaign, fundraise, and train to become PACAs.',
    section2_text4:
      'We need your support again - download the CISTA App and participate in an event near you.',
  },
  pencapaian: {
    section1_text1:
      'The Madani Government has administered Malaysia for just over 6 months.',
    section1_text2:
      'In that short period, multiple initiatives have been rolled out to address the economic pressures that currently weigh down on the Malaysian rakyat.',
    section1_text3:
      'The following snapshot outlines relevant initiatives that the Madani Government has already undertaken to help enable dignified work, dignified wages, and a dignified life',
    section2_text1:
      'If these policy objectives resonate with you, undilah perpaduan. ',
    s1: 'Jobs',
    s2: 'Cost of Living',
    s3: 'Infrastructure',
    s4: 'Welfare',
    s1q1: 'Inisiatif Pendapatan Rakyat (IPR)',
    s1q2: 'Inisiatif Kemudahan Rakyat (IKR)',
    s1q3: 'Hiring of 4,914 Permanent Doctors in MOH',
    s1q4: 'Academy in Factory Initiative',
    s1q5: 'BERNAS Profit-Sharing for Rice Farmers',
    s1q6: 'Subang Airport Regeneration Plan',
    s1q7: 'RM25.5 billion investment from AWS and Tesla',
    s1q8: 'RM30 billion investment from Geely',
    s1q9: 'Termination of PUSPAKOM and MyEG Monopolies',
    s1q10: 'SME Grant Scheme',
    s2q1: 'Electricity Tariff Freeze for Residential Homes and Small Business',
    s2q2: 'Payung Rahmah Initiative',
    s2q3: 'Facilitation of Egg Supply',
    s2q4: '2% Income Tax Reduction for M40',
    s2q5: 'Sumbangan Tunai Rahmah (STR) Incentives',
    s2q6: 'Madani Medical Scheme',
    s2q7: '8% Reduction in BESRAYA and LEKAS Tolls',
    s2q8: 'Free KTM Travel for Primary School Students and OKUs',
    s2q9: 'Reduction of Gas Facility Charge 2023-2025',
    s2q10: 'Seasonal Travel Cost and Capacity Controls',
    s3q1: '40 Flood Mitigation Projects in Kelantan & Terengganu',
    s3q2: '47% of Sick PR1MA Projects Completed in 5 Months',
    s3q3: '7,000 New Affordable Housing Units under SASaR',
    s3q4: "Emergency Funding for 'Sick' PPRs",
    s3q5: 'Transition of Government Buildings to Solar Energy',
    s3q6: 'High-Powered Portable Pumps for Flood Response',
    s3q7: 'RM1.5 billion for Kelantan Water Infrastructure',
    s3q8: 'RM11 billion Savings on ECRL',
    s3q9: 'RM2.8 billion for Prasarana Equipment Replacement',
    s3q10: '7 New EV Charging Stations on PLUS',
    s4q1: '21,000kg of Food Supply for Orang Asli',
    s4q2: 'Citizenship via Malaysian Mothers',
    s4q3: "'Kill Switch' Policy for Banks",
    s4q4: "'Second Chance' Policy for Bankrupts",
    s4q5: '289 Additional Schools Catering to Special Needs Children',
    s4q6: 'New Affordable Housing Quality Standards',
    s4q7: 'Regulation of Vapes',
    s4q8: 'Reactivation of Penang Bumiputera Development Council (PBDC)',
    s4q9: 'RM100 million Flood Allocation for Kelantan and Terengganu',
    s4q10: 'Free Pneumococcal Vaccines for Children',
    s1q1a1:
      "The Madani Government has introduced the People's Income Initiative (IPR) to provide income-earning opportunities that guarantee minimum earnings of RM2,000 to the hardcore poor.",
    s1q1a2:
      "IPR is divided into 3 components. 'INTAN' involves provision of land, equipment, and training to smallholder farmers and a supplementary allowance to ensure RM2,000 monthly income until their first harvest.",
    s1q1a3:
      "'INSAN' involves the provision of vending machines and rental coverage in high-footfall areas such as public transportation hubs, universities, or hospitals. Food vendors are able to make use of these machines at zero cost to sell their products.",
    s1q1a4:
      "'IKHSAN' involves the direct outsourcing of '3D' (Dirty, Dangerous, Difficult) jobs to qualified individuals, bypassing employment agencies who profit from employing foreign labour. IKHSAN workers will also be allowed to collect and keep public fees associated with facilities usage (e.g. public toilets).",
    s1q1a5: 'IPR applications can be made at https://ipr.epu.gov.my/sign_up.',
    s1q2a1:
      "The Madani Government has opened up job opportunities to small independent contractors via the People's Improvements Initiative (IKR).",
    s1q2a2:
      'IKR allows small contractors to identify and apply directly for funding in order to carry out maintenance, improvement, and construction work for small public infrastructure in their local area.',
    s1q2a3:
      'IKR applications can be made at https://ikr.ekonomi.gov.my/register.',
    s1q3a1:
      'The Madani Government has recruited 4,914 permanent positions in government healthcare facilities for 2023 - comprising 4,263 medical officers, 335 dental officers, and 316 pharmacy officers.',
    s1q3a2:
      "To facilitate this increase, Budget 2023 increased MOH's emolument amount by RM3 billion.",
    s1q3a3:
      'The Madani Government is expecting another 12,800 permanent posts to be created and filled by 2026.',
    s1q4a1:
      'The Madani Government launched the Academy in Factory (AIF) training programme initiative that aims to help 100,000 TVET youths gain meaningful work experience and employment.',
    s1q4a2:
      'Working with industry players, AIF provides TVET graduates with relevant on-the-job training in manufacturing fields, prior to absorption into the workforce of participating employers.',
    s1q5a1:
      'In December, the Madani Government directed Bernas to pay out RM10 million in profits to rice farmers, beginning with farmers in Kelantan, Terengganu, and Pahang.',
    s1q5a2:
      'The Madani Government has also directed Bernas to increase this contribution to RM50 million beginning in 2023. Bernas will also start contributing 30% of its net profits to rice farmers.',
    s1q6a1:
      'The Madani Government has approved the reintroduction of commercial jet service from Subang Airport for the first time since 2002.',
    s1q6a2:
      'Beyond facilitating local businesses through greater business footfall, the regeneration plan entails greater investment and job opportunities for maintenance, repair, and overhaul (MRO) centres, aviation research & development centres, and regional commercial flight operations.',
    s1q7a1:
      'The Madani Government secured RM25.5 billion worth of investment from Amazon Web Services (AWS) and Tesla, providing thousands of knowledge-intensive jobs under the main investors and relevant supporting network.',
    s1q7a2:
      "These investments aim to spearhead Malaysia's development as a regional tech hub, leveraging the country's position as a neutral and strategically located nation.",
    s1q8a1:
      'The Madani Government has secured a RM30 billion investment from Geely for the development and commercialisation of the Automotive High-Tech Valley (AHTV) in Tanjong Malim.',
    s1q8a2:
      'The AHTV will establish Malaysia as a regional leader in electric vehicle (EV) research and support services.',
    s1q9a1:
      "The Madani Government has ended PUSPAKOM's exclusive right to conduct vehicle inspection services and MyEG's exlusive right to provide e-government service.",
    s1q9a2:
      'The increased competition aims to reduce waiting times for consumers and increase opportunities for small businesses offering relevant services.',
    s1q10a1:
      'The Madani Government has allocated RM100 million under the SME Grant Scheme to support business automation and digitalisation activities.',
    s1q10a2:
      'Matching grants of up to RM5,000 will be given to SMEs to subscribe to digital products and services, facilitating greater digitalisation and competitiveness of small business. A further RM1 billion has been provided via Bank Negara Malaysia to assist microSMEs through digitalisation and automation schemes.',
    s2q1a1:
      'The Madani Government spent RM10.7 billion in H1 2023 electricity subsidies for MSMEs, restaurants, retail stores, workshops, and agricultural smallholders. ',
    s2q1a2:
      'This initiative ensured that 9.5 million electricity users were shielded from increases in fuel costs in the short-term.',
    s2q2a1:
      'In January, the Madani Government introduced the Payung Rahmah initiative. Working with local businesses and industry players, the Government has worked to expand the supply and sale of affordable products and services to the urban poor.',
    s2q2a2:
      'This includes the introduction of lunch/dinner meals (Menu Rahmah), food baskets, medical treatment (Klinik Rahmah), and eyewear (Pakej Rahmah).',
    s2q3a1:
      'In December, the Madani Government temporarily relaxed import restrictions on foreign eggs in order to address a shortfall in the local supply. This measure was combined with price controls to maintain food costs. These measures were maintained until January, and reintroduced in July in order to offset the shortfall in local egg supply.',
    s2q3a2:
      'The Madani Government has never subsidised the import of eggs, and continues to support local producers with exclusive subsidies.',
    s2q4a1:
      'Under the Madani Government, a 2% income tax reduction has been implemented for those earning between RM35,000 to RM100,000 per annum.',
    s2q4a2:
      'This allows relevant individuals to save up to RM2,300 in annual taxes.',
    s2q5a1:
      'The Madani Government has introduced direct cash aid under the STR initiative.',
    s2q5a2:
      'Households earning less than RM2,500 per month are entitled to up to RM2,500 cash aid (based on number of dependents), with households earning between RM2,501 to RM5,000 per month entitled to up to RM1,250 cash aid (based on number of dependents).',
    s2q5a3:
      'Singles will also be entitled to between RM350 to RM600 based on age.',
    s2q6a1:
      'The Madani Government has also allocated RM100 million for the Madani Medical Scheme, applicable to STR recipients.',
    s2q6a2:
      'STR recipients will be automatically entitled to the Madani Medical Scheme, which provides free consultations, checkups, medicines, and procedures.',
    s2q7a1:
      'The Madani Government enforced an 8% reduction in toll rates on the BESRAYA and LEKAS toll routes, helping lower transportation expenses for thousands of daily commuters between Selangor and Negeri Sembilan.',
    s2q8a1:
      'In February, the Madani Government introduced free travel on KTM commuter and intercity services for students aged up to 12 and people with disabilities.',
    s2q8a2:
      'The cost of free travel is being covered by RM15 million savings achieved through the suspension of the KTM Subang Skypark line.',
    s2q9a1:
      'The Madani Government has reduced the gas facility charge for the period 2023 to 2025.',
    s2q9a2:
      'Depending on the volume of gas consumption and sector, this reduction is projected to translate into savings of between RM4,000 to RM55.9 million for 2023 alone.',
    s2q10a1:
      'Working with industry players, the Madani Government facilitated the increase of 25,020 extra seats from RM199 for the January holiday season.',
    s2q10a2:
      'During the Aidilfitri season, the Madani Government facilitated the addition of 11,400 extra airline seats from RM199, and 13,860 extra ETS seats.',
    s2q10a3:
      'During the Chinese New Year season, the Madani Government also provided regularatory approvals for an extra 291 flights, aimed at addressing capacity shortfall, increasing competition, and reducing airfares.',
    s3q1a1:
      'In December, the Madani Government approved 40 flood mitigation and environmentally friendly drainage projects in Kelantan and Terengganu.',
    s3q1a2:
      'Two major projects - the Sungai Kelantan Integrated River Basin Development Project (PLSB) and the Sungai Golok PLSB are expected to be completed in October 2023 and August 2024 respectively.',
    s3q1a3:
      'The implementation of these projects is designed to increase the number of people protected from the effects of previous floods.',
    s3q2a1:
      "The Madani Government has intervened directly with relevant stakeholders to accelerate the completion of 'sick' PR1MA affordable housing projects.",
    s3q2a2:
      'Between January to May 2023, the Madani Government has succeeded in completing 8 out of 17 identified sick PR1MA projects, with the remainder expected to be complete by 2024.',
    s3q3a1:
      'The Madani Government has made over RM500 million available to maintain and construct Malaysian Armed Forces (MAF) personnel homes, including 7,000 new affordable housing units.',
    s3q4a1:
      "The Madani Government has allocated RM35 million of funding and facilitated the use of limited tender for the 'Kita Untuk Kita' PPR initiative.",
    s3q4a2:
      'The initiative aims to accelerate the repair of faltering PPR infrastructure in Selangor and Kuala Lumpur.',
    s3q5a1:
      'The Madani Government has allocated RM50 million of funding to upgrade and modify government buildings by end of 2023, so that the majority of electricity production is self-generated via solar. ',
    s3q5a2:
      "The Government's goal is to reduce electricity bills and costs borne by the taxpayer, and contribute to Malaysia's ongoing green transition.",
    s3q6a1:
      'In December, the Madani Government spent RM12 million to provide 6 high-powered portable pumps to the Kelantan State Government. This was done in order to aid in the removal of stagnant floodwater in Pasir Mas.',
    s3q7a1:
      'The Madani Government has approved a combined RM1.5 billion in funding to resolve water supply issues in Kelantan.',
    s3q8a1:
      'The Madani Government announced the reduction in development cost of the ECRL from RM85.97 billion to RM74.96 billion, after the implementation of total value engineering (VM).',
    s3q8a2:
      'By proceeding with the ECRL at a reduced cost, the Madani Government hopes to increase connectivity on a 665km stretch between Kota Bharu and Port Klang.',
    s3q9a1:
      'To address increasing cases of equipment failure, the Madani Government has provided Prasarana Malaysia with a RM2.8 billion allocation in order to replace faulty train/bus equipment and enable stockpiling of spare parts.',
    s3q10a1:
      'Under the Madani Government, RM90 million has been allocated over a two-year period to build direct charging stations for EVs along the North-South Highway. ',
    s3q10a2:
      'These stations will augment the existing stations in Ayer Keroh, Tapah, and Paka.',
    s4q1a1:
      'The Madani Government authorised the supply of 21,000kg of food to 1,400 Orang Asli families in Kelantan.',
    s4q1a2:
      'The distribution of this food supply was undertaken by the Fire and Rescue Department, making use of helicopter equipment as villages were inaccessible via land after flooding. ',
    s4q2a1:
      'The Madani Government has begun the process of amending Part III of the Federal Constitution to allow children born to Malaysian mothers abroad to obtain Malaysian citizenship.',
    s4q2a2:
      'Separately, the Government has already resolved 8,288 out of 10,000 citizenship appeal cases.',
    s4q3a1:
      "The Madani Government has instructed Bank Negara to enforce a 'kill switch' policy for all banking institutions, enabling users to immediately freeze their accounts and cards in the event of suspicious activity.",
    s4q3a2:
      'The move has been pursued as a measure combat online fraud and scams.',
    s4q4a1:
      "The Madani Government has implemented a 'Second Chance' policy for insolvents in Malaysia.",
    s4q4a2:
      'Bankruptcy cases below RM50,000 have been automatically discharged, with amendments to the Insolvency Act passed in order to allow speedier discharge of bankruptcies in the future.',
    s4q5a1:
      'The Madani Government has increased the number of primary and secondary schools offering the Integrated Special Needs Programme from 2,343 to 2,632.',
    s4q5a2:
      "This will allow further enforcement of the 'zero reject policy', and ensure that special needs children throughout the country have access to education.",
    s4q6a1:
      "The Madani Government has mandated that 220,000 new or incomplete affordable housing projects must abide by certain 'livability' and 'comfort' standards.",
    s4q6a2:
      'Meeting these standards involve passing windtunnel tests, sun-path analysis, and thermal comfort analysis.',
    s4q7a1:
      'The Madani Government has legalised e-cigarettes and vapes, allowing for the legal distribution and tighter regulation of such devices.',
    s4q7a2:
      'This will enable the Government to enforce minimum health & safety standards on relevant products, and more closely monitor and intervene on vape-related health issues.',
    s4q8a1:
      "The Madani Government has reactivated the PBDC under the Prime Minister's Department. ",
    s4q8a2:
      'The PBDC works closely with the Penang State Government to accelerate the development of the Penang Malay community, and focuses on the development of wakaf land and bumiputera property ownership.',
    s4q9a1:
      'The Madani Government allocated RM100 million to Kelantan and RM100 million to Terengganu to provide flood relief to those affected by the December floods.',
    s4q10a1:
      'The Madani Government has introduced a one-year period of free pneumococcal vaccinations for children born between 2018 and 2019.',
    s4q10a2:
      'This initiative seeks to close the gap present in the previous National Immunisation Programme, whereby children born between 2018 and 2019 were not covered.',
  },
  volunteer: {
    section1_text1: 'Tawaran Madani is driven by people like you.',
    section1_text2:
      'If you would like to join our field teams — calling fence-sitters & canvassing, training to become PACAs — download the CISTA App and register today. Perpaduan is driven by people like you.',
  },
  faq: {
    s1q1: 'How can I register as a volunteer for the Undi Perpaduan campaign?',
    s1q2: 'I have registered as a volunteer. What activities can I do after this?',
    s2q1: 'How can I download CISTA?',
    s2q2: "I'm having trouble registering the CISTA application.",
    s2q3: "I can't login into the CISTA app.",
    s2q4: 'CISTA app registration takes a long time to complete.',
    s2q5: 'Why does CISTA app registration require identity card detail?',
    s2q6: 'I am having trouble downloading the CISTA app outside of Malaysia.',
    s1a1: 'To become a volunteer, you are required to register as a volunteer via the CISTA app.',
    s1a2: 'Any latest activities regarding Undi Perpaduan will be updated on the CISTA app.',
    s2a1: 'CISTA can be downloaded via the App Store, Play Store & Huawei App Gallery.',
    s2a2: "Make sure you enter a valid phone number & identity card number before pressing the 'Register Account' button.",
    s2a3: "Make sure the phone number and password you enter are correct before you press the 'Sign In' button.",
    s2a4: 'Make sure you have a stable internet connection.',
    s2a5: 'Identity card detail is required for registration purposes and PACA activities.',
    s2a6: 'As of now, the CISTA app is only available in Malaysia.',
  },
};
